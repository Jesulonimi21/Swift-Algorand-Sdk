...
public class LogicsigSignatureTests:XCTestCase{
    ...
    public func testLogicsigCreation() throws {
        ...
        XCTAssert(lsig.sig==nil)
        XCTAssert(lsig.msig==nil)
        var outBytes:[UInt8] = try CustomEncoder.encodeToMsgPack(lsig)
        var lsig1  = try CustomEncoder.decodeFrmMessagePack(obj: LogicsigSignature.self, data: Data(outBytes))
        XCTAssertEqual(lsig, lsig1)
        try TestUtil.serializeDeserializeCheck(object: lsig);
    }
    
    public func testLogicsigInvalidProgramCreation() throws {
    ...
    public func testLogicsigSignature() throws {
        var program:[Int8] = [1,32,1,1,34,]
        var lsig = try LogicsigSignature(logicsig: program)
...
//        print(jsonString)
    ...
    public func testLogicsigMultisigSignature() throws {
        var verified = false
        var program:[Int8] = [1,32,1,1,34,]
        ...
        verified = try lsig.verify(address: try ma.toAddress());
        ...
        verified = try lsig.verify(address: account.getAddress())
        XCTAssertFalse(verified)
        ...
        var outBytes:[UInt8] = try CustomEncoder.encodeToMsgPack(lsig)
        var lsig2 = try CustomEncoder.decodeFrmMessagePack(obj: LogicsigSignature.self, data: Data(outBytes))
        XCTAssertEqual(lsig2, lsig)
        verified = try lsig2.verify(address: try ma.toAddress())
        XCTAssertTrue(verified)
        try TestUtil.serializeDeserializeCheck(object: lsig);
    }
