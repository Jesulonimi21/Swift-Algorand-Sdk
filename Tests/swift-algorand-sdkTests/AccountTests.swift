...
public class AccountTests : XCTestCase{
    func testSignsTransactionE2E()throws{
        ...
        var stxBytes:[Int8] = try  CustomEncoder.encodeToMsgPack(stx)
        var signedTxHex = try  CustomEncoder.encodeToHexString(bytes:stxBytes)
    ...
    func testSignTransactionZeroValE2E()throws{
    ...
    func testToMnemonic()throws{
        var FROM_SK = "actress tongue harbor tray suspect odor load topple vocal avoid ignore apple lunch unknown tissue museum once switch captain place lemon sail outdoor absent creek"
        var seed = try Mnemonic.toKey(FROM_SK)
        var account = try Account(seed)
        XCTAssertEqual( try account.toMnemonic(), FROM_SK)
        
    }

    private func makeTestMsigAddr()throws->MultisigAddress{
    ...
    func testSignMultisigTransaction()throws{
        var addr = try makeTestMsigAddr()
        ...
        var enc:[Int8] = try  CustomEncoder.encodeToMsgPack(stx)
        XCTAssertEqual(try account.signMultisigTransactionBytes(from: addr, tx: tx), enc)
    ...
    public func testAppendMultisigTransaction()throws{
        var addr = try makeTestMsigAddr()
        ...
        var account = try Account(seed)
        var appended = try account.appendMultisigTransactionBytes(from: addr, txBytes: CustomEncoder.convertToInt8Array(input: firstTxBytes))
    ...
    func testSignMultisigKeyRegTransaction()throws{
        var addr = try makeTestMsigAddr()
        ...
        var wrappedTx = try CustomEncoder.decodeFrmMessagePack(obj: SignedTransaction.self, data: Data(encKeyRegTx))
        var seed = try Mnemonic.toKey("auction inquiry lava second expand liberty glass involve ginger illness length room item discover ahead table doctor term tackle cement bonus profit right above catch")
        var account = try Account(seed)
    ...
    func testAppendMultisigKeyRegTransaction()throws{
        var addr = try makeTestMsigAddr()
    ...
    func testMergeMultisigTransaction ()throws{
        ...
        var a = try Account.mergeMultisigTransactionBytes(txsBytes:[CustomEncoder.convertToInt8Array(input: firstAndThird) ,CustomEncoder.convertToInt8Array(input: secondAndThird) ])
    
        var b = try Account.mergeMultisigTransactionBytes(txsBytes:[CustomEncoder.convertToInt8Array(input: secondAndThird), CustomEncoder.convertToInt8Array(input: firstAndThird)]);
        XCTAssertEqual(a, b)
    ...
    func testTransactionSignatureVerification()throws{
        var mnemonic="cactus check vocal shuffle remember regret vanish spice problem property diesel success easily napkin deposit gesture forum bag talent mechanic reunion enroll buddy about attract"
                   ...
                   .build()
        var account = try Account(mnemonic)
        var stxBytes:[UInt8] = account.keyPair.sign( try CustomEncoder.encodeToMsgPack(tx))
        var isVerified = try account.keyPair.verify(signature: stxBytes, message:try  CustomEncoder.encodeToMsgPack(tx))
      
       try AlgoLogic.loadLangSpec()
        XCTAssertEqual( isVerified, true)
            
        
    }
    func testLogicsigTransaction()throws{
     var from = try Address("47YPQTIGQEO7T4Y4RWDYWEKV6RTR2UNBQXBABEEGM72ESWDQNCQ52OPASU")
       ...
       var args:[[Int8]] = Array()
        ...
        var stx = try Account.signLogicsigTransaction(lsig: lsig, tx: tx)
        var messagePack:[Int8] = try  CustomEncoder.encodeToMsgPack(stx)
    ...
    func testTealSign() throws {
        ...
        var sig1 = try acc.tealSign(data:data,contractAddress:addr)
        var sig2 = try acc.tealSignFromProgram(data: data, program: prog)
